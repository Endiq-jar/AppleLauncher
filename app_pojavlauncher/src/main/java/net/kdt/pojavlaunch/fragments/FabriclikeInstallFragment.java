...
public abstract class FabriclikeInstallFragment extends Fragment implements ModloaderDownloadListener, CompoundButton.OnCheckedChangeListener {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        mGameVerList.setOnItemClickListener((parent, v, position, id) -> {
            ...
            Log.d("AppleLauncher", "Game Version selected: " + mSelectedGameVersion);
            ...
            Log.d("AppleLauncher", "Triggering loader version fetch for: " + mSelectedGameVersion);
    ...
    private void onClickStart(View v) {
        ...
        mStartButton.setEnabled(false);
        Log.d("AppleLauncher", "Installation Started");
    ...
    public void onDownloadFinished(File downloadedFile) {
        ...
        Tools.runOnUiThread(()->{
            if (!isFragmentUiAvailable()) return;
            Log.d("AppleLauncher", "Installation Finished");
    ...
    class LoaderVersionSelectedListener implements AdapterView.OnItemSelectedListener {
        ...
        public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
            ...
            Log.d("AppleLauncher", "Selected Loader Version = " + mSelectedLoaderVersion);
            mStartButton.setEnabled(true);
        }
    ...
    class LoadLoaderVersionsTask implements SelfReferencingFuture.FutureInterface {
        ...
        public void run(Future<?> myFuture) {
            Log.i("AppleLauncher", "LoadLoaderVersionsTask START for game version: " + mSelectedGameVersion);
            try {
                mLoaderVersionArray = mFabriclikeUtils.downloadLoaderVersions(mSelectedGameVersion);
                if (mLoaderVersionArray != null) {
                    Log.i("AppleLauncher", "LoadLoaderVersionsTask SUCCESS: loaded " + mLoaderVersionArray.length + " loader versions");
                    onFinished(myFuture);
                } else {
                    Log.e("AppleLauncher", "LoadLoaderVersionsTask FAILED: downloadLoaderVersions returned null");
                    onException(myFuture, null);
                }
            } catch (IOException e) {
                Log.e("AppleLauncher", "LoadLoaderVersionsTask FAILED with IOException", e);
        ...
        private void onFinished(Future<?> myFuture) {
            Tools.runOnUiThread(()->{
                if(myFuture.isCancelled() || !isFragmentUiAvailable()) {
                    Log.w("AppleLauncher", "LoadLoaderVersionsTask onFinished: skipped (cancelled or UI unavailable)");
                    return;
                }
                Log.d("AppleLauncher", "LoadLoaderVersionsTask onFinished: updating loader spinner");
                stopLoading();
                updateLoaderSpinner();
            });
        }
    ...
    private void updateLoaderSpinner() {
        if(!isFragmentUiAvailable() || mLoaderVersionArray == null) {
            Log.w("AppleLauncher", "updateLoaderSpinner: skipped (UI unavailable or no loader array)");
            return;
        }
        mLoaderVersionSpinner.setAlpha(0f);
        ArrayAdapter<FabricVersion> adapter = createAdapter(mLoaderVersionArray, false, true);
        if (adapter == null) {
            Log.w("AppleLauncher", "updateLoaderSpinner: adapter is null");
            return;
        }
        Log.d("AppleLauncher", "Game Version = " + mSelectedGameVersion);
        Log.d("AppleLauncher", "updateLoaderSpinner: created adapter with " + adapter.getCount() + " items");
        ...
        if (mLoaderVerList != null) {
            ...
            Log.d("AppleLauncher", "updateLoaderSpinner: attached adapter to loader ListView");
        } else {
            Log.w("AppleLauncher", "updateLoaderSpinner: mLoaderVerList is null!");
        }
        ...
        if (adapter.getCount() > 0) {
            ...
            Log.d("AppleLauncher", "updateLoaderSpinner: auto-selected loader " + mSelectedLoaderVersion);
        } else {
            mSelectedLoaderVersion = null;
            mSelectedLoaderPosition = -1;
            Log.w("AppleLauncher", "updateLoaderSpinner: adapter is empty");
        }
    ...
    class GameVersionSelectedListener implements AdapterView.OnItemSelectedListener {
        ...
        public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
            ...
            Log.d("AppleLauncher", "Selected MC Version = " + mSelectedGameVersion);
            cancelFutureChecked(mLoaderVersionFuture);
            updateLoaderVersions();
        }
    ...
    class LoadGameVersionsTask implements SelfReferencingFuture.FutureInterface {
        @Override
        public void run(Future<?> myFuture) {
            Log.i("AppleLauncher", "LoadGameVersionsTask START");
            try {
                mGameVersionArray = mFabriclikeUtils.downloadGameVersions();
                if(mGameVersionArray != null) {
                    Log.i("AppleLauncher", "LoadGameVersionsTask SUCCESS: loaded " + mGameVersionArray.length + " game versions");
                    onFinished(myFuture);
                } else {
                    Log.e("AppleLauncher", "LoadGameVersionsTask FAILED: downloadGameVersions returned null");
                    onException(myFuture, null);
                }
            }catch (IOException e) {
                Log.e("AppleLauncher", "LoadGameVersionsTask FAILED with IOException", e);
                onException(myFuture, e);
            }
        }
        private void onFinished(Future<?> myFuture) {
            Tools.runOnUiThread(()->{
                if(myFuture.isCancelled() || !isFragmentUiAvailable()) {
                    Log.w("AppleLauncher", "LoadGameVersionsTask onFinished: skipped (cancelled or UI unavailable)");
                    return;
                }
                Log.d("AppleLauncher", "LoadGameVersionsTask onFinished: updating game spinner");
                stopLoading();
                updateGameSpinner();
            });
        }
