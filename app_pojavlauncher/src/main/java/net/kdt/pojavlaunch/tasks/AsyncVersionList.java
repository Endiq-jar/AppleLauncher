...
public class AsyncVersionList {
    ...
    public void getVersionList(@Nullable VersionDoneListener listener, boolean secondPass){
        sExecutorService.execute(() -> {
            ...
            // A stale/corrupted cache file can parse as valid JSON but contain no
            // actual versions (e.g. an interrupted download, or an error page that
            // got saved instead of the real manifest). Treat that the same as a
            // parse failure: wipe the bad cache and force a fresh download, rather
            // than silently handing the UI a permanently empty list.
            boolean isEmpty = versionList == null || versionList.versions == null || versionList.versions.length == 0;
            if (isEmpty && !secondPass) {
                Log.w("AsyncVersionList", "Version list came back empty, forcing a fresh download");
                versionFile.delete();
                getVersionList(listener, true);
                return;
            }
